...
int fill_trees(TTree *ttree_hits, TTree *signal, TTree *background, TTree *sig_tst, TTree *bg_tst, int RunNum, int *rtw1, int *rtw3) {
  ...
  #else
    fChain->SetBranchAddress("nhit", &tgem_nhit, &b_tgem_nhit);
    fChain->SetBranchAddress("nclu", &tgem_nclu, &b_tgem_nclu);
  #endif
  //========================================
...
*/        
      ...
      printf("Run %d not found in the PS 2025 range - exiting \n",RunNum);
      exit(1);
    }
  ...
  #if ANALYZE_MERGED
    #ifdef MMG_RUN
      sprintf(G_DIR,"mlpOutput/ps25/merged/hd_rawdata_mmg_%06d_%06dEntries.root",RunNum,nEntries);
    #else
      sprintf(G_DIR,"mlpOutput/ps25/merged/hd_rawdata_%06d_%06dEntries.root",RunNum,nEntries);
    #endif
  #else
    #ifdef MMG_RUN
      sprintf(G_DIR,"mlpOutput/ps25/hd_rawdata_mmg_%06d.root",RunNum);
    #else
      sprintf(G_DIR,"mlpOutput/ps25/hd_rawdata_%06d.root",RunNum);
    #endif
  #endif
  ...
  #else
  l1->AddEntry(hNhits, "tgem_nhits");
  l1->AddEntry(hNclu, "tgem_nclu");
  #endif
  l1->Draw();
...
void trd_mlp_ps25(int RunNum) {
  ...
  #else
    sprintf(rootfile,"RootOutput/ps25/trd_singleTrackHits_Run_%06d.root",RunNum);
  #endif
  char basename[200];
  ...
  #if ANALYZE_MERGED
    #ifdef MMG_RUN
      sprintf(mlpname,"mlpOutput/ps25/merged/mlp_mmg_run%06d_%06dEntries.root",RunNum,nEntries);
    #else
      sprintf(mlpname,"mlpOutput/ps25/merged/mlp_run%06d_%06dEntries.root",RunNum,nEntries);
    #endif
  #else
    #ifdef MMG_RUN
      sprintf(mlpname,"mlpOutput/ps25/mlp_mmg_run%06d.root",RunNum);
    #else
      sprintf(mlpname,"mlpOutput/ps25/mlp_run%06d.root",RunNum);
    #endif
  #endif
